    ...
    void init() {
        ...
        for (uint8_t i = 0; i < cpus.size(); i++) {
            ...
            if(!enabled_bit && !online_cpbl_bit) {
                ...
                io::printf("CPU %u is unable to be started\n", apic_id);
                continue;
            }
            ...
            */
            ...
            if(fully_started) {
                ...
                io::printf("Failed to start CPU %d. Reason: %s\n", apic_id, reason_str);
            }
        }

        /* CLearing up - deleting all of the trampoline code from the first usable page */
        // memset128((void*) trampoline_addr, 0, 4096 / 16);
